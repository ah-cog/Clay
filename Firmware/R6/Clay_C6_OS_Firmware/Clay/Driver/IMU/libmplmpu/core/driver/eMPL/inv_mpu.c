...
#elif defined EMPL_TARGET_CLAY_R5
#include "Clock.h"
...
#define log_e(...)
#define reg_int_cb 
#define i2c_write write_mpu9250
#define i2c_read read_mpu9250
...
#elif defined MPU6500
const struct gyro_reg_s reg =
{  //must be initialzed in same order defined in struct definition for c++,
		who_am_i : 0x75,         //otherwise get a 'sorry' message saying that 'non-trivial designated initializers are not supported'
				rate_div : 0x19, lpf : 0x1A, prod_id : 0x0C, user_ctrl : 0x6A, fifo_en : 0x23, gyro_cfg : 0x1B, accel_cfg : 0x1C, accel_cfg2 : 0x1D, lp_accel_odr : 0x1E, motion_thr : 0x1F, motion_dur : 0x20, fifo_count_h : 0x72, fifo_r_w : 0x74, raw_gyro : 0x43, raw_accel : 0x3B, temp : 0x41, int_enable : 0x38, dmp_int_status : 0x39, int_status : 0x3A, accel_intel : 0x69, pwr_mgmt_1 : 0x6B, pwr_mgmt_2 : 0x6C, int_pin_cfg : 0x37, mem_r_w : 0x6F, accel_offs : 0x77, i2c_mst : 0x24, bank_sel : 0x6D, mem_start_addr : 0x6E, prgm_start_h : 0x70
#ifdef AK89xx_SECONDARY
				, s0_addr : 0x25, s0_reg : 0x26, s0_ctrl : 0x27, s1_addr : 0x28, s1_reg : 0x29, s1_ctrl : 0x2A, s4_ctrl : 0x34, s0_do : 0x63, s1_do : 0x64, i2c_delay_ctrl : 0x67, raw_compass : 0x49
#endif
		};
const struct hw_s hw =
{ .addr = 0x68, .max_fifo = 1024, .num_reg = 128, .temp_sens = 321, .temp_offset = 0, .bank_size = 256
#if defined AK89xx_SECONDARY
		, .compass_fsr = AK89xx_FSR
#endif
		};
...
const struct test_s test =
{ .gyro_sens = 32768 / 250, .accel_sens = 32768 / 2,        //FSR = +-2G = 16384 LSB/G
		.reg_rate_div = 0, /* 1kHz. */
...
#if !defined EMPL_TARGET_STM32F4 && !defined EMPL_TARGET_CLAY_R5    
	if (int_param)
		...
		for (i = 0; i < 3; i++)
		{
			...
			if (debug)
			{
				log_i("Bias_Shift=%7.4f, Bias_Reg=%7.4f, Bias_HWST=%7.4f\r\n",
						...
						bias_st[i]/1.f); log_i("OTP value: %7.4f\r\n", ct_shift_prod[i]/1.f);
			}
...
int mpu_write_mem(unsigned short mem_addr, unsigned short length, unsigned char *data)
{
	...
	//hack: timing shit. super specific for the load_firmware function.
	delay_ms(1);
	...
	//write high byte of address to bank select, low byte to mem_start_addr
	...
	//write data to the memory read/write register.
