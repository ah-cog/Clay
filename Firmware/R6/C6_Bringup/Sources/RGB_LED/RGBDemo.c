...
void RGBDemoLoop()
{
   ...
   for (;;)
   {
      ...
      if (!(Millis() % gTime))
      {
         if (rDescending)
         {
            if (LastState != ButtonPressToggler || (r -= rStep) <= rMin)
            {
               rDescending = FALSE;
               if (!gDescending && !bDescending)
               {
                  LastState = ButtonPressToggler;
               }
            }
         }
         else
      ...
      if (!(Millis() % gTime))
      {
         if (gDescending)
         {
            if (LastState != ButtonPressToggler || (g -= gStep) <= gMin)
            {
               gDescending = FALSE;
      ...
      if (!(Millis() % bTime))
      {
         if (bDescending)
         {
            if (LastState != ButtonPressToggler || (b -= bStep) <= bMin)
            {
               bDescending = FALSE;
