...
static uint32_t Iser0LastVaue;
static uint32_t Iser1LastVaue;
static uint32_t Iser2LastVaue;
static uint32_t Iser3LastVaue;

int8_t Enable_ESP8266()
{
...
void ESP8266_Buffer_Incoming_Data()
{
    ...
    {        // Check if any data has been received
        ...
        while (ESP8266_Get_Incoming_Buffer_Size() > 0)
        {        // Read each of the received characters from the buffer and send them to the device.
            unsigned char ch;
 ...
 */
int8_t ESP8266_Search_Tail_For_Response(const char *response, const char *buffer, unsigned int bufferSize)
{
    // DEBUG: printf ("\t\tESP8266_Search_For_Response \"%s\" in \"%s\" (bufferSize: %d)\r\n", response, buffer, bufferSize);

    if (bufferSize >= strlen(response))
...
int8_t ESP8266_Wait_For_Response(const char* response, uint32_t milliseconds)
{
    ...
    for (incomingDataQueueSize = 0; incomingDataQueueSize < HTTP_RESPONSE_BUFFER_SIZE; incomingDataQueueSize++)
    {
        incomingDataQueue[incomingDataQueueSize] = (char) 0;
    }        // Initialize the buffer. This might not be necessary.
    incomingDataQueueSize = 0;
    uint8_t incomingBufferSize = 0;
...
//		incomingBufferSize = ESP8266_Get_Incoming_Buffer_Size ();
        ...
        {        // Check if any data has been received
            // DEBUG: printf ("incomingBufferSize (before) = %d\r\n", incomingBufferSize);
            while (Ring_Buffer_NofElements() > 0)
            {        // Read each of the received characters from the buffer and send them to the device.
                unsigned char ch;
                ...
                if (bufferStatus == ERR_OK)
                {
                    ...
                    {        // Check if the IPD message header has been received...

                        if (receivingHeader == FALSE)
                        {        // ...if not, check if it is receiving the header. If not look for it.
                        ...
                        else if (receivingHeader == TRUE)
                        {        // ...if so, start buffering it and extract parameters as they're received to direct further queueing (i.e., data flow) operations.
                            ...
                            if (ch == ':')
                            {
                                ...
                                incomingDataQueue[6] = '\0';        // Terminate the connection ID string by replacing the ',' following the connection ID with NULL.
                                ...
                                connectionID = (int8_t)atoi(&incomingDataQueue[5]);        // Point to the connection byte
...
int8_t ESP8266_Send_Command_AT_CWMODE(uint8_t mode)
{
    ...
    sprintf(buffer, "AT+CWMODE_CUR=%d\r\n", mode);
...
{        // TODO: Set_Internet_Address (ESP8266_Profile *hardwareProfile)
    // Copy substring starting at location pointed to by stationIP. The IP address of the station is the substring from "STAIP,\"" to the following "\"". 
    //stationIP = stationIP + strlen ("STAIP,\""); // Go to start of IP address substring
    //length = strchr (stationIP, '"') - stationIP;
    //stationIP[length] = '\0';
...
//			D(printf ("Joining access point. "));
            ...
            if ((status = ESP8266_Send_Command_AT_CWJAP(internetProfile.wifi_ssid, internetProfile.wifi_password)) > 0)
            {        // if ((status = ESP8266_Send_Command_AT_CWJAP (esp8266_profile.wifi_ssid, esp8266_profile.wifi_password)) > 0) { // if ((status = ESP8266_Send_Command_AT_CWJAP (SSID_DEFAULT, PASSWORD_DEFAULT)) > 0) {
 ...
 */
int8_t Start_HTTP_Server(uint16_t port)
{
...
//		// ESP8266_Receive_Incoming_Request ("\r\n\r\n", ESP8266_DEFAULT_RESPONSE_TIMEOUT); // i.e., "0,CONNECT\r\n\r\n"
...
//			printf ("Getting state of channel.\r\n");
            ...
            i = strchr(httpUri, '?') - httpUri;        // Computes the offset between the beginning of the URI and the message content.
            if (i > 0)
            {        // The offset will be greater than 0 if the '?' character was found.
...
static void Receive_HTTP_Request(int8_t connection)
{
...
//	printf ("Operation: %s\r\n", operation);
    ...
    tokenEnd = strchr(httpMethod, ' ');
    *tokenEnd = '\0';        // Terminate the string.
    // TODO: Search for the method (i.e., "GET", "POST", etc.) and set a flag.
    // printf ("Method: %s\r\n", httpMethod);
...
static void Receive_Datagram(int8_t connection)
{
...
//    int8_t connection = -1;        // 0 to 4
    ...
    messageContent[(connectionDataQueueSize[connection])] = '\0';        // message[incomingDataQueueSize] = NULL;
...
//			D(printf("connectionDataQueue[connection] = %s\r\n", connectionDataQueue[connection]));
            ...
            if (strstr(connectionDataQueue[connection], "HTTP/1.1") != NULL)
            {        // if (strstr (incomingDataQueue, "HTTP/1.1") != NULL) {
                Receive_HTTP_Request(connection);
            }
            else
            {

                Receive_Datagram(connection);
...
//	D(printf ("--> UDP Broadcast: %s\r\n", buffer));
    ...
    n = sprintf(buffer, "POST /experiment HTTP/1.1\r\nHost: %s:%d\r\nContent-Type: text/plain\r\nContent-Length: %d\r\n\r\n%s", address, remotePort,
            strlen(message), message);

    // Broadcast data to the broadcast address.
    // e.g., n = sprintf (buffer, "AT+CIPSEND=%d,%d,\"%s\",%d\r\n", channel, strlen ("HELLO\r\n"), remoteAddress, remotePort);
//	n = sprintf (buffer, "AT+CIPSEND=%d,%d,\"%s\",%d\r\n", channel, strlen (message), remoteAddress, remotePort);
    //n = sprintf (buffer, "AT+CIPSEND=%d,%d\r\n", channel, (100 + strlen (message)));
    n = sprintf(buffer, "AT+CIPSEND=%d,%d\r\n", channel, n);
    if (ESP8266_Send_Block(buffer) == TRUE)
    {
        // Wait for ">" before sending data.
...
//	n = sprintf (buffer, "%s", "POST /experiment HTTP/1.1\r\nHost: %s:%d\r\nContent-Type: text/plain\r\nContent-Length: %d\r\n\r\n%s", address, remotePort, strlen (message), message);
    n = sprintf(buffer, "POST /experiment HTTP/1.1\r\nHost: %s:%d\r\nContent-Type: text/plain\r\nContent-Length: %d\r\n\r\n%s", address, remotePort,
            strlen(message), message);
    if (ESP8266_Send_Block(buffer) == TRUE)
    {
