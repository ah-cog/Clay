...
int8_t Perform_Behavior(Behavior *behavior)
{
	...
	// turn light 1 on
	// ^
	if ((status = getToken(behaviorContent, token, 0)) != NULL)
	{ // status = getToken (message, token, 0);
		...
		else if (strncmp(token, "change", strlen("change")) == 0)
		{
			...
			if ((status = getToken(behaviorContent, token, 1)) != NULL)
			{
				...
				if (strncmp(token, "channel", strlen("channel")) == 0)
				{
					...
					if ((status = getToken(behaviorContent, token, 2)) != NULL)
					{ // TODO: Remove this! The parameter is used later.
