...
Message* Set_Message_Content(const char *content, uint32_t content_length) {
...
// Allocate memory for the message's content.
   (*message).content = (char *) malloc(content_length);
   memset((*message).content, 0, content_length);
