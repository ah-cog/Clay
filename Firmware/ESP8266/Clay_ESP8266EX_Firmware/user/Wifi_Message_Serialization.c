...
uint32_t Serialize_Message(Message * message, uint8_t * destination_string,
		uint32_t destination_max_length)
{
	...
	rval = snprintf(destination_string, destination_max_length,
			"  %s%s%s%s%s%s%s%s%s%d%s", message_start, message->type,
...
Message * Deserialize_Message(uint8_t * message)
{
	...
	uint8_t * temp_content = temp_content_length + strlen(temp_content_length)
			+ 1;

	content_length = atoi(temp_content_length);

	if (temp_type != NULL && temp_source_address != NULL
			&& temp_dest_address != NULL && temp_content_type != NULL
			&& temp_content_length != NULL && temp_content != NULL)
	{
