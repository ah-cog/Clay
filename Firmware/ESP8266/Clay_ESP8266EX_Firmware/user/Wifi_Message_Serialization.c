...
////Globals   /////////////////////////////////////////////////////
uint8_t* message_type_strings[] =
{ "udp", "tcp", "command", "status", "http", "invalid" };

uint8_t* content_type_strings[] =
{ "text", "bin", "invalid" };
...
Message_Type Get_Message_Type_From_Str(char * typeString)
{
	...
	{
		if (strcmp(typeString, message_type_strings[i]) == 0)
		{
			rval = (Message_Type) i;
		}
	}
...
uint32_t Serialize_Message_With_Message_Header(Message * message,
		uint8_t * destination_string, uint32_t destination_max_length)
{

	if (message == NULL || destination_string == NULL)
		return 0;
	...
	rval = snprintf(destination_string, destination_max_length,
			"%s%05d%s%05d%s%s%s%s%s%s%s%d%s%d%s%s%s",     //
			message_start,     //
			...
			message_field_delimiter, message->message_type,
			message_field_delimiter, message->source, message_field_delimiter,
			message->destination, message_field_delimiter,
			message->content_length, message_field_delimiter,
			message->content_checksum, message_field_delimiter,
			message->content_type, message_field_delimiter);

	if ((rval) + message->content_length <= destination_max_length)
	{

		memcpy(destination_string + rval, message->content,
				message->content_length);
		rval += message->content_length;

		sprintf(destination_string + 1, "%05d", rval); //write the length after the first char (plus 2 for the padding at the start of the string, + 1 for the delimiter. -2 from size for the padding.)
		destination_string[6] = *message_field_delimiter; //put our delimiter back.

		//f00000t00000t
		message_checksum = Calculate_Checksum_On_Bytes(destination_string + 13,
				rval - 13);
...
uint32_t Serialize_Message_Content(Message * message,
		uint8_t * destination_string, uint32_t destination_max_length)
{
...
Message * Deserialize_Message_With_Message_Header(uint8_t * message)
{
	...
	if (message == NULL || message[0] != *message_start)
		return NULL;
	...
	//get framing info
	uint8_t * temp_message_length = (uint8_t*) strtok_r((char*) message + 1,
			message_field_delimiter, &token_state); //throw out the start character
	...
	if (temp_message_length != NULL)
	{
		...
		temp_message_checksum = strtok_r(NULL, message_field_delimiter,
				&token_state);
	...
	if (temp_message_length != NULL && temp_message_checksum != NULL
			&& message_checksum
					== Calculate_Checksum_On_Bytes(
							temp_message_checksum
									+ strlen(temp_message_checksum) + 1,
							message_length
									- ((temp_message_checksum
											+ strlen(temp_message_checksum) + 1)
											- message)))
	{
		...
		uint8_t * temp_message_type = strtok_r(NULL, message_field_delimiter,
				&token_state);
		uint8_t * temp_source_address = strtok_r(NULL, message_field_delimiter,
				&token_state);
		uint8_t * temp_dest_address = strtok_r(NULL, message_field_delimiter,
				&token_state);
		uint8_t * content_header_start = temp_dest_address
				+ strlen(temp_dest_address);

		if (temp_message_type != NULL && temp_source_address != NULL
				&& temp_dest_address != NULL && content_header_start != NULL)
		{
...
Message * Deserialize_Message_Content(uint8_t * message)
{
	...
	if (message == NULL || *message != *message_start)
		return NULL;
	...
	//find content header
	uint8_t * temp_content_length = strtok_r(message + 1,
			message_field_delimiter, &token_state);
	uint8_t * temp_content_checksum = strtok_r(NULL, message_field_delimiter,
			&token_state);
	uint8_t * temp_content_type = strtok_r(NULL, message_field_delimiter,
			&token_state);

	content_length = atoi(temp_content_length);

	if (temp_content_length != NULL && temp_content_type != NULL
			&& temp_content_checksum != NULL)
	{

		//get a pointer to the content
		uint8_t * temp_content = temp_content_type
				+ (strlen(temp_content_type) + 1);
