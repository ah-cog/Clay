...
LDD_TDeviceData* IMU_IRQ_DeviceData;

static struct platform_data_s gyro_pdata = { .orientation = { 1, -1, 0, 1, 0, 0,
		0, 0, 1 } };
...
static uint8_t Init_MPU9250();

uint8_t Enable_MPU9250() {
	uint8_t rval = 1;
	return rval;
}
...
uint8_t Start_MPU9250() {
	...
	rval = Init_MPU9250();
	IMU_IRQ_DeviceData = IMU_IRQ_Init(IMU_IRQ_DeviceData);
	IMU_IRQ_SetPortEventCondition(IMU_IRQ_DeviceData, (uint32_t)(1 << 9), LDD_GPIO_FALLING);

	return rval;
}

uint8_t Stop_MPU9250() {
	uint8_t rval = 1;
	return rval;
}

uint8_t Pause_MPU9250() {
	uint8_t rval = 1;
	
	IMU_IRQ_SetPortEventCondition(IMU_IRQ_DeviceData, (uint32_t)(1 << 9), LDD_GPIO_DISABLED);
	
	return rval;
}

uint8_t Resume_MPU9250() {
	uint8_t rval = 1;
...
void imu_periodic_callback() {
	...
	status = dmp_read_fifo(v.values.gyro.array, v.values.accel.array, q.array,
			&gyro_timestamp, &sensors, &more);
