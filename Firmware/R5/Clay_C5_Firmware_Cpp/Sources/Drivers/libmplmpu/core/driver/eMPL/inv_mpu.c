...
#define get_ms(x) *x = power_on_time_msec
...
#elif defined MPU6500
const struct gyro_reg_s reg = {  //must be initialzed in same order defined in struct definition for c++, 
...
struct chip_cfg_s chip_cfg_instance;          //see comment below -- GBH

static struct gyro_state_s st = {             //must be initialzed in same order defined in struct definition for c++,                             
        reg : &reg,                           //otherwise get a 'sorry' message saying that 'non-trivial designated initializers are not supported'
        hw : &hw,                             //chip cfg is init'd here for that reason.
        chip_cfg : chip_cfg_instance,
        test : &test
};
#endif
