...
void APP_Run (void) {
	...
	// Connect to Wi-Fi (Send the data to the bridged serial peripheral (UART0))
	SendString ((unsigned char*) "AT+CWSAP=\"Clay AP\",\"1234567890\",5,3\r\n", &deviceData);
	SendString ((unsigned char*) "AT+RST\r\n", &deviceData);
	SendString ((unsigned char*) "AT+CWMODE=3\r\n", &deviceData);
	SendString ((unsigned char*) "AT+CWJAP=\"clay-2.4ghz\",\"goldenbrown\"", &deviceData);
	SendString ((unsigned char*) "AT+CIFSR\r\n", &deviceData);
	SendString ((unsigned char*) "AT+CIPMUX=1\r\n", &deviceData);
	SendString ((unsigned char*) "AT+CIPSERVER=1,80\r\n", &deviceData);
