...
/*lint -restore Enable MISRA rule (6.3) checking. */
{
    ...
    RH_NRF24 * radio = new RH_NRF24(MESH_CE_PIN_INDEX, 0, hardware_spi);
    RHMesh * meshRouter = new RHMesh(*radio, (uint8_t) 0);

    meshRouter->init();
    meshRouter->setThisAddress(1);

    mpu_values local_imu_data = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
    mpu_values remote_imu_data = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };

    color_rgb colors[] = { { LED_MODE_OFF, LED_MODE_OFF, LED_MODE_OFF },        //off
    ...
    for (;;)
    {
        if (tick_1msec)
        {
            ...
            {                
                ...
                meshRouter->sendtoWait((uint8_t*) &local_imu_data,
                                    sizeof(mpu_values), 2, 0);                
